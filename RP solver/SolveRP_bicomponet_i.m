%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                                                               %
%       RP Equation Solver - 2 components - solve inner layer   %
...
rho_gas2liq = find_sat(water,'rho_liq',Tinf);
rho_liq = find_sat(hexane,'rho_liq',Tinf);
pgas = find_sat(water,'pressure',Tinf)*1e3 * rho_gas2liq / rho_gas;
...
rho = rho_liq;
surface_tension = find_sat(hexane,'surface_tension',Tinf);
viscosity = find_sat(hexane,'viscosity',Tinf)*1e-6;
...
% R0 = 2*surface_tension/(pgas - pinf);
% R0 = R0*(1+1e-3);
y0(1) = R0i;
y0(2) = 0.;
...
% Solve the equation
options = 0;
[T,Y] = ode45(@RP_adiabatic_i, [0 evolution_time], y0);
